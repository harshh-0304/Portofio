...
function MobileSkillGroup({ skill, defaultOpen }: { skill: (typeof skills)[0]; defaultOpen: boolean }) {
  ...
  return (
    <div className="jarvis-panel rounded-2xl overflow-hidden">
      <button
      ...
      >
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: open ? "rgba(0,212,255,0.15)" : "rgba(0,212,255,0.05)" }}
          >
            <Icon size={17} className={open ? "text-cyan-400" : "text-cyan-500/40"} />
          </div>
          <div className="text-left">
            <div className={`text-sm font-mono font-bold tracking-wider ${open ? "text-cyan-300" : "text-cyan-500/50"}`}>
              {skill.category.toUpperCase()}
            </div>
            <div className="text-[10px] font-mono text-cyan-500/30">{skill.items.length} MODULES</div>
          </div>
        </div>
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-4 pb-4 pt-3 space-y-4 border-t border-cyan-500/10">
              {skill.items.map((item, i) => (
                <div key={item.name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm font-mono text-cyan-200/80">{item.name}</span>
                  ...
                  <div className="h-1.5 rounded-full skill-bar-track relative overflow-hidden">
                    <motion.div
                      ...
                      style={{
                        ...
                        boxShadow: item.level >= 90 ? "0 0 8px rgba(0,255,136,0.5)" : "0 0 6px rgba(0,212,255,0.4)",
                      }}
                      initial={{ width: 0 }}
  ...
  return (
    <section ref={ref} className="py-16 md:py-24 px-4">
      <div className="max-w-6xl mx-auto">
        {/* Section header */}
        <motion.div className="mb-10 md:mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <h2 className="text-3xl md:text-4xl font-bold font-mono neon-cyan">SKILL MATRIX</h2>
          <p className="text-cyan-500/40 font-mono text-sm mt-2 tracking-widest">
            <span className="hidden md:inline">RUNNING SYSTEM ANALYSIS — SELECT CATEGORY</span>
            <span className="md:hidden">TAP A CATEGORY TO EXPAND</span>
          </p>
        </motion.div>
        ...
        <div className="lg:hidden space-y-3">
          {skills.map((skill, i) => (
            <MobileSkillGroup key={skill.category} skill={skill} defaultOpen={i === 0} />
          ))}
        </div>
        ...
        <div className="hidden lg:grid grid-cols-3 gap-8">
          {/* Category tabs */}
          <motion.div className="space-y-3" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            {skills.map((skill, i) => {
              const Icon = iconMap[skill.icon] ?? Server;
              ...
              return (
                ...
                >
                  <div className="w-8 h-8 rounded flex items-center justify-center shrink-0" style={{ background: isActive ? "rgba(0,212,255,0.15)" : "rgba(0,212,255,0.05)" }}>
                    <Icon size={16} className={isActive ? "text-cyan-400" : "text-cyan-500/40"} />
                  </div>
                  <div>
                    <div className={`text-sm font-mono font-bold tracking-wider ${isActive ? "text-cyan-300" : "text-cyan-500/50"}`}>{skill.category.toUpperCase()}</div>
                    <div className="text-xs font-mono text-cyan-500/30">{skill.items.length} MODULES</div>
                  </div>
                  {isActive && <motion.div className="ml-auto w-1 h-6 rounded-full bg-cyan-400" layoutId="active-tab" style={{ boxShadow: "0 0 8px rgba(0,212,255,0.6)" }} />}
                </motion.button>
              );
            })}
          </motion.div>
          ...
          {/* Skill bars */}
          <motion.div className="lg:col-span-2 jarvis-panel rounded-xl p-6" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <div className="space-y-5">
              {activeSkills.items.map((item, i) => (
                <motion.div key={`${activeSkills.category}-${item.name}`} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: i * 0.08 }}>
                  ...
                  <div className="h-1.5 rounded-full skill-bar-track relative overflow-hidden">
                    <motion.div className="absolute top-0 left-0 h-full rounded-full"
                      style={{ background: item.level >= 90 ? "linear-gradient(90deg, #00FF88, #00D4FF)" : item.level >= 80 ? "linear-gradient(90deg, #00D4FF, #0ea5e9)" : "linear-gradient(90deg, rgba(0,212,255,0.7), rgba(0,212,255,0.4))", boxShadow: item.level >= 90 ? "0 0 8px rgba(0,255,136,0.5)" : "0 0 8px rgba(0,212,255,0.4)" }}
                      initial={{ width: 0 }} animate={{ width: animated ? `${item.level}%` : 0 }} transition={{ duration: 1.2, delay: i * 0.1, ease: "easeOut" }} />
                    <motion.div className="absolute top-0 h-full w-8 rounded-full opacity-60"
                      style={{ background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent)" }}
                      animate={{ left: ["-10%", `${item.level + 5}%`] }} transition={{ duration: 1.5, delay: i * 0.1 + 0.8, ease: "easeOut" }} />
                  </div>
                </motion.div>
              ))}
            </div>
            <div className="mt-6 pt-4 border-t border-cyan-500/10 flex items-center gap-6">
              {[{ color: "from-green-400 to-cyan-400", label: "Expert (90%+)" }, { color: "from-cyan-400 to-blue-400", label: "Advanced (80%+)" }, { color: "from-cyan-400/70 to-cyan-400/40", label: "Proficient" }].map((l) => (
